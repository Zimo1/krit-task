/* ИСПРАВЛЕННЫЙ ВАРИАНТ (изменена только таблица конвертации единиц измерения t_convertation. Алгоритм полностью прежний)
...
public class SolutionCorrectedSettings {
            ...
            for (String r: p_resources) {
                ...
                for (String y: p_years) {
                    ...
                    while (true) {
                        ...
                        if (m_calculated.isEmpty() && m_based.isEmpty() && m_result.isEmpty()) break;
                            ...
                            for (T_values m_value : m_values) {
                                for (T_multiplication based : m_based) {
                                    if (m_value.measureIncludedInCalculatedMeasure(based)) {
                                        ...
                                        // 5.1.1.5.2.2. Записать в T_values для I, R, Y все значения, рассчитанные в
